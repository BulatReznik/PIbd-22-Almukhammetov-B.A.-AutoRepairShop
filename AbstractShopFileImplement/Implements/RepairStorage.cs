...
namespace RepairFileImplement.Implements
{
    public class RepairStorage : IRepairStorage
    {
        ...
        public List<RepairViewModel> GetFilteredList(RepairBindingModel model)
        {
            ...
            .Where(rec => rec.RepairName.Contains(model.RepairName)).Select(CreateModel).ToList();
        }
        public RepairViewModel GetElement(RepairBindingModel model)
        {
            ...
            var repair = source.Repairs.FirstOrDefault(rec => rec.RepairName == model.RepairName || rec.Id == model.Id);
